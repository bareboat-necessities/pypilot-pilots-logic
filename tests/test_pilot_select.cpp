...
int main() {
    ...
    model.ap.pilot.value = PilotName::simple;

    PilotResult result = compute_selected_pilot(model, 10);
    assert(result.valid);
    assert(std::fabs(result.command_norm - (10.0f * 0.005f + 1.0f * 0.15f)) < 0.0001f);
    return 0;
}
